...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['process_return'])) {
    ...
    $check = $conn->prepare("SELECT request_id, status, date_needed, student_id FROM borrow_requests WHERE request_id = ?");
    ...
    if (!$request) {
        ...
        if ($return_date > $request['date_needed']) {
            $penalty_reason .= "Late return ";
            $penalty_amount += 50;
        }
        if (strtolower($condition_on_return) === 'damaged') {
            $penalty_reason .= "Damaged apparatus";
            $penalty_amount += 100;
        }
        ...
        if ($penalty_amount > 0 && isset($t['transaction_id'])) {
            ...
            sendPenaltyNotification($request['student_id'], $penalty_amount, $penalty_reason);
        }
    ...
    <div class="card">
        ...
        <form method="POST" class="row g-3 mb-4">
            <div class="col-md-3">
                ...
                <input type="number" name="request_id" class="form-control" required min="1"
