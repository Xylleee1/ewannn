...
if ($role === 'admin' || $role === 'assistant') {
  ...
  $stmt = mysqli_prepare($conn, "SELECT COUNT(*) as total FROM borrow_requests WHERE faculty_id=?");
  mysqli_stmt_bind_param($stmt, 'i', $user_id);
  mysqli_stmt_execute($stmt);
  $result = mysqli_stmt_get_result($stmt);
  ...
  $stmt = mysqli_prepare($conn, "SELECT COUNT(*) as total FROM borrow_requests WHERE student_id=? AND status='approved'");
  mysqli_stmt_bind_param($stmt, 'i', $user_id);
  mysqli_stmt_execute($stmt);
  $result = mysqli_stmt_get_result($stmt);
    ...
    <?php while ($activity = mysqli_fetch_assoc($recent_activities)): ?>
      <div class="activity-item">
        <div class="activity-text">
          ...
          if (strpos($action_lower, 'request') !== false)
            $badge_class = 'badge-request';
          elseif (strpos($action_lower, 'approve') !== false)
            $badge_class = 'badge-approve';
          elseif (strpos($action_lower, 'reject') !== false)
