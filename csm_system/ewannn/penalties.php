...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_penalty'])) {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        // Get transaction_id from transaction_code using prepared statement
        $stmt_txn = mysqli_prepare($conn, "
            SELECT t.transaction_id, br.student_id 
            FROM transactions t 
            JOIN borrow_requests br ON t.request_id = br.request_id 
            WHERE t.transaction_code LIKE ? LIMIT 1
        ");
        $txn_search = '%' . $transaction_code . '%';
        mysqli_stmt_bind_param($stmt_txn, 's', $txn_search);
        mysqli_stmt_execute($stmt_txn);
        $txn_result = mysqli_stmt_get_result($stmt_txn);
        ...
        if ($txn_result && mysqli_num_rows($txn_result) > 0) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                // Send email and notification immediately
                if (isset($txn_row['student_id'])) {
                    sendPenaltyNotification($txn_row['student_id'], $amount, $reason);
                }
            } else {
...
// Handle Mark as Paid (POST only with CSRF)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_paid'])) {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        $stmt = mysqli_prepare($conn, "UPDATE penalties SET status='paid' WHERE penalty_id=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_penalty'])) {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        $stmt = mysqli_prepare($conn, "DELETE FROM penalties WHERE penalty_id=?");
        mysqli_stmt_bind_param($stmt, 'i', $penalty_id);
        mysqli_stmt_execute($stmt);
...
<div class="penalty-card">
    ...
    <form method="POST" class="row g-3 mb-4 add-form">
        ...
        <div class="col-md-3">
            <input type="text" name="transaction_id" class="form-control" placeholder="Transaction Code" required
                pattern="[A-Za-z0-9\-]+">
        </div>
        <div class="col-md-3">
        ...
        <div class="col-md-2">
            <input type="number" step="0.01" name="amount" class="form-control" min="0.01" placeholder="Amount"
                required>
        </div>
        <div class="col-md-2">
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="action-btns">
                                    ...
                                    <?php endif; ?>
                                    <button class="btn-edit btn-sm" data-bs-toggle="modal"
                                        data-bs-target="#editModal<?= $p['penalty_id']; ?>">
                                        <i class="bi bi-pencil"></i>
                                    </button>
                                    <form method="POST" style="display:inline;"
                                        onsubmit="return confirm('Delete this penalty?')">
                                        <input type="hidden" name="csrf_token" value="<?= generate_csrf_token() ?>">
                                        <input type="hidden" name="penalty_id" value="<?= $p['penalty_id'] ?>">
                                        <button type="submit" name="delete_penalty" class="btn-delete btn-sm">
                                    ...
                                    <div class="modal-dialog">
                                        <form method="POST" class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <input type="hidden" name="penalty_id" value="<?= $p['penalty_id']; ?>">
                                                <input type="text" name="reason" value="<?= htmlspecialchars($p['reason']); ?>"
                                                    class="form-control mb-2" required>
                                                <input type="number" step="0.01" name="amount" value="<?= $p['amount']; ?>"
                                                    class="form-control mb-2" min="0.01" required>
                                                <input type="date" name="date_imposed" value="<?= $p['date_imposed']; ?>"
