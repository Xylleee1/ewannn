...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if (mysqli_num_rows($check_result) > 0) {
                    ...
                    if (mysqli_stmt_execute($stmt)) {
                        ...
                        if ($role === 'student') {
                            ...
                            mysqli_stmt_bind_param($stmt2, "isss", $new_user_id, $first_name, $last_name, $middle_initial);
    ...
    if (isset($_POST['edit_user'])) {
        ...
        if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
            ...
            mysqli_stmt_bind_param($stmt, "ssssi", $username, $email, $full_name, $role, $edit_user_id);
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if ($role === 'student') {
                    ...
                    $stmt_check_student = mysqli_prepare($conn, "SELECT user_id FROM students WHERE user_id = ?");
                    mysqli_stmt_bind_param($stmt_check_student, 'i', $edit_user_id);
                    mysqli_stmt_execute($stmt_check_student);
                    $check_student = mysqli_stmt_get_result($stmt_check_student);
                    if (mysqli_num_rows($check_student) > 0) {
                        $stmt2 = mysqli_prepare($conn, "UPDATE students SET first_name=?, last_name=?, middle_initial=? WHERE user_id=?");
                        mysqli_stmt_bind_param($stmt2, "sssi", $first_name, $last_name, $middle_initial, $edit_user_id);
                        mysqli_stmt_execute($stmt2);
                    } else {
                        $stmt2 = mysqli_prepare($conn, "INSERT INTO students (user_id, first_name, last_name, middle_initial) VALUES (?, ?, ?, ?)");
                        mysqli_stmt_bind_param($stmt2, "isss", $edit_user_id, $first_name, $last_name, $middle_initial);
                        mysqli_stmt_execute($stmt2);
                    }
                    mysqli_stmt_close($stmt_check_student);
                } else {
                    // If role changed from student to something else, delete from students
                    $stmt_del = mysqli_prepare($conn, "DELETE FROM students WHERE user_id = ?");
                    mysqli_stmt_bind_param($stmt_del, 'i', $edit_user_id);
                ...
                $msgClass = "success";
                add_log($conn, $_SESSION['user_id'], "Edit User", "Updated user ID: $edit_user_id");
            } else {
    ...
    if (isset($_POST['reset_password'])) {
        ...
        if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
            ...
            if (!validate_password($new_password)) {
                ...
                mysqli_stmt_bind_param($stmt, "si", $hashed_password, $reset_user_id);

                if (mysqli_stmt_execute($stmt)) {
                    $message = "Password reset successfully!";
                    $msgClass = "success";
                    add_log($conn, $_SESSION['user_id'], "Reset Password", "Reset password for user ID: $reset_user_id");
                }
            }
        }
    }
    ...
    if (isset($_POST['delete_user'])) {
        if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
            ...
            if ($del_user_id == $_SESSION['user_id']) {
                ...
                $stmt_check = mysqli_prepare($conn, "SELECT request_id FROM borrow_requests WHERE faculty_id = ? OR student_id = ? LIMIT 1");
                mysqli_stmt_bind_param($stmt_check, 'ii', $del_user_id, $del_user_id);
                mysqli_stmt_execute($stmt_check);
                $check_borrow = mysqli_stmt_get_result($stmt_check);

                if (mysqli_num_rows($check_borrow) > 0) {
                    $message = "Cannot delete user because they are referenced in borrow requests.";
                    $msgClass = "danger";
                } else {
                    $stmt_del = mysqli_prepare($conn, "DELETE FROM users WHERE user_id = ?");
                    mysqli_stmt_bind_param($stmt_del, 'i', $del_user_id);
                    mysqli_stmt_execute($stmt_del);
                    $message = "User deleted successfully.";
                    $msgClass = "success";
                    add_log($conn, $_SESSION['user_id'], "Delete User", "Deleted user ID: $del_user_id");
                }
                mysqli_stmt_close($stmt_check);
            }
        }
...
if (!empty($search)) {
    $search_param = '%' . $search . '%';
    $where_clauses[] = "(u.username LIKE ? OR u.full_name LIKE ? OR u.email LIKE ? OR CONCAT(s.last_name, ' ', s.first_name, ' ', s.middle_initial) LIKE ?)";
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="action-btns">
                                    <button class="btn-edit" onclick='showEditModal(<?= json_encode($u) ?>)'><i
                                            class="bi bi-pencil"></i> Edit</button>
                                    <button class="btn-password" onclick="showPasswordModal(<?= $u['user_id'] ?>)"><i
                                            class="bi bi-key"></i> Reset</button>
                                    <?php if ($u['user_id'] != $_SESSION['user_id']): ?>
                                        <form method="POST" style="display:inline;" onsubmit="return confirm('Delete this user?')">
                                            <input type="hidden" name="csrf_token" value="<?= generate_csrf_token() ?>">
                                            <input type="hidden" name="user_id" value="<?= $u['user_id'] ?>">
                                            <button type="submit" name="delete_user" class="btn-delete"><i class="bi bi-trash"></i>
                                                Delete</button>
                                        </form>
                                    <?php endif; ?>
                                </div>
...
<div id="addModal" class="modal">
    <div class="modal-content">
        ...
        <form method="POST">
            ...
            <button type="submit" name="add_user" class="btn-add w-100"><i class="bi bi-check-circle"></i> Add
                User</button>
        </form>
    </div>
</div>
...
<div id="editModal" class="modal">
    <div class="modal-content">
        ...
        <form method="POST" id="editForm">
            ...
            <div class="form-group"><label>Username</label><input type="text" name="username" id="edit_username"
                    required></div>
            <div class="form-group"><label>Email</label><input type="email" name="email" id="edit_email" required></div>
            <div class="form-group"><label>Full Name</label><input type="text" name="full_name" id="edit_full_name">
            </div>
            <div class="form-group"><label>Role</label>
            ...
            <button type="submit" name="edit_user" class="btn-add w-100"><i class="bi bi-save"></i> Save
                Changes</button>
        </form>
    </div>
</div>
    ...
    <div class="modal-content">
        ...
        <form method="POST">
            ...
            <div class="form-group"><label>New Password</label><input type="password" name="new_password" required
                    minlength="8" placeholder="Minimum 8 characters"></div>
            <button type="submit" name="reset_password" class="btn-add w-100"><i class="bi bi-check-circle"></i> Reset
                Password</button>
        </form>
    </div>
</div>
